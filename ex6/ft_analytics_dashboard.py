...
def main() -> None:
    ...
    high_scorers = [player for player, score in player_scores.items() if score > 2000]
    ...
    elite_players_upper = [player.upper() for player, score in player_scores.items() 
                           if score > 2000]
    ...
    score_categories = {
        ...
        'medium': sum(1 for score in player_scores.values() if 1800 <= score < 2000),
    ...
    score_to_player = {score: player for player, score in player_scores.items()}
    ...
    player_metrics = {
        player: {
            ...
            'achievements': len(player_achievements.get(player, []))
        }
        for player in all_unique_players
    }
    

    top_scorer = max(player_scores.items(), key=lambda x: x[1])
    top_achiever = max(player_achievements.items(), key=lambda x: len(x[1]))
